...
export default {
    ...
    methods: {
        applyActiveClass(userIterado) {
            return {
               'table-active': this.selectedUser && this.selectedUser.userEmail === userIterado.userEmail
            }
        },
        getAll() {
             return [
        ...
        excluir(us) {
            const indice = this.users.findIndex(user => user.userEmail === us.userEmail)
            this.users.splice(indice, 1)
            this.selectedUser = undefined
            this.edit = false
        },
        updateUser(updatedUser) {
            const indice = this.users.findIndex(user => user.userEmail === updatedUser.userEmail)
